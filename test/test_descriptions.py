import unittest

import os
import codecs

import ubx
from ubx.descriptions import mon_hw
from ubx.descriptions import rxm_rawx


class TestPrinting(unittest.TestCase):
    def test_AtomicVariable(self):
        self.assertEqual(str(ubx.payload.U1), "U1")
        self.assertEqual(str(ubx.payload.U2), "U2")
        self.assertEqual(str(ubx.payload.U4), "U4")
        self.assertEqual(str(ubx.payload.U8), "U8")

        self.assertEqual(str(ubx.payload.I1), "I1")
        self.assertEqual(str(ubx.payload.I2), "I2")
        self.assertEqual(str(ubx.payload.I4), "I4")
        self.assertEqual(str(ubx.payload.I8), "I8")

        self.assertEqual(str(ubx.payload.X1), "Bitfield(1)")
        self.assertEqual(str(ubx.payload.X2), "Bitfield(2)")
        self.assertEqual(str(ubx.payload.X4), "Bitfield(4)")
        self.assertEqual(str(ubx.payload.X8), "Bitfield(8)")

        self.assertEqual(str(ubx.payload.R4), "R4")
        self.assertEqual(str(ubx.payload.R8), "R8")

    def test_List(self):
        self.assertEqual(str(ubx.payload.List(["a", "b"])), "[\n  a,\n  b\n]")
        self.assertEqual(str(ubx.payload.List(["a", ubx.payload.List("b")])),
                        "[\n"
                        "  a,\n"
                        "  [\n"
                        "    b\n"
                        "  ]\n"
                        "]")

    def test_Fields(self):
        field1 = ("a", "va")
        field2 = ("b", "vb")
        field3 = ("c", "vc")
        fields1 = ubx.payload.Fields(field1, field2)
        self.assertEqual(str(fields1),
                        "Fields(length=?) {\n"
                        "  a: va,\n"
                        "  b: vb\n"
                        "}")
        fields2 = ubx.payload.Fields(("fields", fields1), field3)
        self.assertEqual(str(fields2),
                        "Fields(length=?) {\n"
                        "  fields:\n"
                        "    Fields(length=?) {\n"
                        "      a: va,\n"
                        "      b: vb\n"
                        "    },\n"
                        "  c: vc\n"
                        "}")

    def test_Loop(self):
        field1 = ("a", "va")
        field2 = ("b", "vb")
        field3 = ("c", "vc")
        self.assertEqual(str(ubx.payload.KeyLoop("k", "va")),
                        "Loop(key=\"k\"):\n"
                        "| va")
        self.assertEqual(str(ubx.payload.KeyLoop("k", ubx.payload.Fields(field1, field2, field3))),
                        "Loop(key=\"k\"):\n"
                        "| Fields(length=?) {\n"
                        "|   a: va,\n"
                        "|   b: vb,\n"
                        "|   c: vc\n"
                        "| }")



class TestDescriptions(unittest.TestCase):
    pass


def make_test_read_serialize(directory, name):
    _, message_class, message_id, length, checksum = name[:-4].split("-")
    classbyte = ubx.utils.hexstring2byte(message_class)
    try:
        message_class = ubx.message_class.get_by_byte(classbyte)
    except ValueError:
        message_class = ubx.message_class.MessageClass("?", classbyte)
    message_id = codecs.decode(message_id.encode("utf-8"), "hex")
    checksum = codecs.decode(checksum.encode("utf-8"), "hex")
    def test(self):
        with open(os.path.join(directory, name), "rb") as f:
            reader = ubx.Reader(f.read)
            msg = reader.read_rawmessage()
        self.assertEqual(message_class, msg.message_class)
        self.assertEqual(message_id, msg.message_id)
        self.assertEqual(int(length), len(msg))
        self.assertEqual(checksum, msg.checksum().bytes())
        with open(os.path.join(directory, name), "rb") as f:
            data = f.read()
        self.assertEqual(data, msg.serialize())
    return test


testdir = "ubx-testdata/data"
names = os.listdir(testdir)
for name in names:
    if not name.endswith("ubx"):
        continue
    f = make_test_read_serialize(testdir, name)
    setattr(TestDescriptions, 'test_{0}'.format(name[:-4]), f)


if __name__ == '__main__':
    unittest.main()
